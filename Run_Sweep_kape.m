...
P.ga2   = 0.2;   % sysP(11)
%% 2) kap_e 扫描列表（建议跨数量级）
kape_list = [0.005, 0.01, 0.02, 0.05, 0.1];
...
title(ax1, 'Upper Mass FRF vs \kappa_e');

ax2 = subplot(1,2,2); hold(ax2,'on'); grid(ax2,'on'); box(ax2,'on');
xlabel(ax2, '\Omega'); ylabel(ax2, '20log_{10}(|Q|) (dB)');
title(ax2, 'Circuit State |Q| vs \kappa_e');

legend_str = cell(numel(kape_list),1);

fprintf('================================================\n');
fprintf('SWEEP: kap_e = %s\n', mat2str(kape_list));
fprintf('Fw = %.4f, lambda = %.4f\n', Fw, P.lam);
fprintf('kap_c = %.4f, sigma = %.4f\n', P.kap_c, P.sigma);
fprintf('================================================\n');

PeakTable = [];  % [kap_e, mode_id, Omega_peak, AmpX1_dB_peak]

for k = 1:numel(kape_list)
    P.kap_e = kape_list(k);

    sysP = [P.be1, P.be2, P.mu, P.al1, P.ga1, P.ze1, P.lam, ...
            P.kap_e, P.kap_c, P.sigma, P.ga2];

    fprintf('\n>>> Case %d/%d: kap_e = %.6f\n', k, numel(kape_list), P.kap_e);
    ...
    legend_str{k} = sprintf('\\kappa_e = %.3g', P.kap_e);
    ...
    if ~isempty(pk_dB)
        for m = 1:numel(pk_dB)
            PeakTable = [PeakTable; P.kap_e, m, pk_w(m), pk_dB(m)]; %#ok<AGROW>
